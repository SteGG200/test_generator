...
def create_exam_document(path: str, exam_content: str):
	...
	while current_line_number <= len(lines):
		...
		if type_current_line == -1:
			raise ValueError(f'In "{path}/{CONTENT_FILE}" on line {current_line_number}:\nUnexpected line')
			...
			while buffer_line_number <= len(lines) and line_detect(lines[buffer_line_number - 1]) == 0:
			...
			if type_current_line == 2: # Normal option
				for index, content_line in enumerate(content_lines):
					if line_detect(content_line) == 0:
						...
						if len(array_option) != 2:
							raise ValueError(f'In "{path}/{CONTENT_FILE}" on line {current_line_number + index}:\nInvalid option format')
						order_option, content_option = array_option
						question_paragraph.add_run(f'\n{order_option.upper()}. ').bold = True
						question_paragraph.add_run(content_option)
				if buffer_line_number <= len(lines) and line_detect(lines[buffer_line_number - 1]) == 1:
				...
				for index, content_line in enumerate(content_lines):
					if line_detect(content_line) == 0:
						...
						order_option, content_option = array_option
						order_option_runner = question_paragraph.add_run(f'\n{order_option.upper()}. ')
				...
				for index, content_line in enumerate(content_lines):
					if len(content_line) > 0 and content_line[0].isnumeric():
						...
						question_paragraph.add_run(f'Câu {number_question}: ').bold = True
